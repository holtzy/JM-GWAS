...
#PBS -N trait
#PBS -A UQ-IMB-CNSG
#PBS -o /shares/compbio/Group-Yang/a.xue/GSMR/UKB_GWAS_summary/trait/stdout_gwas_trait
#PBS -e /shares/compbio/Group-Yang/a.xue/GSMR/UKB_GWAS_summary/trait/stderr_gwas_trait
#PBS -l select=1:ncpus=16:intel=True:mem=96GB
#PBS -l walltime=168:00:00

cd /shares/compbio/Group-Yang/a.xue/GSMR/UKB_GWAS_summary/

if [ ! -d "trait" ]; then
    mkdir trait
fi

cd /shares/compbio/Group-Yang/a.xue/GSMR/UKB_GWAS_summary/trait/
mv *.* /shares/compbio/Group-Yang/a.xue/trash/

bolt="/shares/compbio/Group-Yang/a.xue/bin/BOLT/bolt"
phen="/gpfs/gpfs01/polaris/Q0286/uqaxue/phen/ukb_v2_22_diseases_match_gera.phen"
...
out="/shares/compbio/Group-Yang/a.xue/GSMR/UKB_GWAS_summary/trait/ukbEUR_MAF1e-4_trait.bolt"

$bolt --bed=$bed --bim=$bim --fam=$fam --exclude $exsnp --phenoFile=$phen --phenoCol=trait --covarFile $covar --covarCol=Sex --qCovarCol=Age --qCovarCol=PC{1:10} --lmm --modelSnps=$j_hm3 --maxModelSnps=720000 --LDscoresF
ile=$table --geneticMapFile=$map --statsFile=$out --numThreads 16 >> trait_bolt.log 2>&1
```
...
```{bash, eval=FALSE}
# first I need to compute the allele frequency of the reference allele with gcta (~30 minutes)
# See plink script, I will use the same output

# Now I can build the .ma file for GSMR
echo "SNP A1 A2 freq b se p n" > GWAS_blood_pressure_bolt.ma
join <(awk '{print $2, $5, $6}' /gpfs/gpfs01/polaris/Q0286/UKBiobank/v2EURu_HM3/ukbEURu_imp_chr*bim | sort -k 1,1) <(awk '{print $2, $6}' /home/y.holtz/BLOOD_GWAS/1_GWAS/PLINK/all_frequency*.frq | sort) | join - <(zcat result_GWAS_bloodpressure_bolt.linear.gz | awk '{if(NR>1) print $2, $7, $7/$8, $9, $6 }' | sort -k 1,1) >> GWAS_blood_pressure_bolt.ma
```
