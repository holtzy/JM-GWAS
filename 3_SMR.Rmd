...
A summary of what we have in the eQTL data:
...
## GC | chr4
Do we have eQTL in the region of the GC gene? Between 71 and 74 Mb ?

- Westra: an eQTL for the gene RUFY3
- eQTLGen: 5 eQTL. ENSG00000080493 = SLC4A4 = gene just before GC. But controled by SNP rs12330929 that is NOT in our vitaminD hit area.
- gtex: nothing

**Conclusion**: we have **NO chance** to detect GC as associated in this SMR analysis.
...
eqtlgen %>% 
  filter(topSNP_chr==4) %>%
  filter(topSNP_bp>71000000 & topSNP_bp<73000000)
```
...
## CYP2R1 | chr11 | ~15 Mb
Do we have eQTL in the region of the CYP2R1 gene? Between 14 and 15.5 Mb ?

- Westra: 3 eQTLs: for COPB1, PDE3B and INSC.
- eQTLGen: 5 eQTL: for RRAS2, COPB1, PDE3B, **CYP2R1** and INSC. CYP2R1 is our candidate gene since the top hit SNP is in this gene
- gtex: 1 eQTl: SPON1, but far before our QTL.

**Conclusion**: we have a chance to detect something with SMR.
...
- ENSG00000133818 = RRAS2
- ENSG00000129083 = COPB1
- ENSG00000152270 = PDE3B
- ENSG00000186104 = CYP2R1 --> our main target! controled by SNP rs2060793
- ENSG00000188487 = INSC (Just after the famous CYP2R1)
...
Do we have eQTL in the region of the DHCR7 gene? Between 70 and 72 Mb ?

- Westra: 3 eQTLs: for IL18BP, NUMA1, C11ORF59 but I don't get it cause this genes are not in the LocusZoom figure
- eQTLGen: 4 eQTL: for DHCR7, AP002387.1, NADSYN1, FAM86C1. Exactly our targets!
- gtex: 1 eQTl: PPFIA1 and RP11-660L16.2
...
gtex %>% 
  filter(topSNP_chr==11) %>%
  filter(topSNP_bp>70000000 & topSNP_bp<73000000)
```
...
#- Detail of the chromosome 11 ~15Mb {.tabset}
***

Can we check what happend in the chromosome 11 area without multiple testing correction? Do we find new associations

## Westra
Only One association already described before
```{r}
westra %>% 
  filter(p_SMR < 0.05) %>%
  filter(p_SMR > (0.05 / nrow(westra)) ) %>%
  filter(topSNP_chr==11) %>%
  filter(topSNP_bp>11000000 & topSNP_bp<19000000)
```


## eQTLGEN
6 associations are under 0.05 but did not survive multiple testing correction in this area:
```{r}
eqtlgen %>% 
  filter(p_SMR < 0.05) %>%
  filter(p_SMR > (0.05 / nrow(eqtlgen)) ) %>%
...
-ENSG00000188487 = INSC (Just after the famous CYP2R1) --- Heidi test OK  
-ENSG00000166689 = PLEKHA7 --- Heidi test OK  
-ENSG00000011405 = PIK3C2A --- Heidi test OK  
-ENSG00000070081 = NUCB2 --- Heidi test OK  
-ENSG00000260196 = AC124798.1  
