---
title: "An overview of the Vitamin D GWAS sumstat from Xia et al."
...
##Manhattan
The genetic determinism of Vitamin-D appears to be **oligogenic** with about 6 loci involved. The association on chromosome 4 is impressive, with a -log(p-value) > 300.
...
##Clumping
First of all I can run the [plink clumping algorithm](http://zzz.bwh.harvard.edu/plink/clump.shtml) on this GWAS summary statistics to hae an insight of the unique association loci. Here are the clumping options I've used. Note: GWAS summary file must NOT be zipped.
...
tmp_command="plink \
          ...
          --clump-p1 0.00000001 --clump-p2 0.01 --clump-r2 0 --clump-kb 500 \
...
The clumping results are not satisfying yet. I've got several loci where I'm suppose to have only one. I do not have anything on chromosome 14 where I have a significant association.



##FUMA
A FUMA analysis of this GWAS sumstat has been run. The result is accessible [here](http://fuma.ctglab.nl/snp2gene/9026). 
...
<br><br>
<div align="center">
![Figure: Association between gene expression and phenotype trough genotypes. (a) A model of cqusqlity where a difference in phenotype is caused by a diffference in genotype mediated by gene expression (transcription). (b) Three possible explanations for an observed association between a trait and gene expression through genotypes. From Zhihong Zhu et al.](IMG/smr_explanation.png)
</div>
<br><br>

If an association between a gene and VitaminD is detected, several option exist (figure b): 
...
Note: need to understand the HEIDI story.
