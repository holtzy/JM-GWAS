...
# Merge
data <- merge(PRS, GPS, by.x="ID", by.y="f.eid", all.x=T) %>% na.omit() %>% select(longitude, latitude, PRS, ID, f.31.0.0)
...
#- Relationship with diseases {.tabset}
***
...
```{r, eval=FALSE}
# Read files
all <- read.table("list_all", header=TRUE)
C92 <- read.table("list_C92", header=FALSE)
M313 <- read.table("list_M313", header=FALSE)

# Build bilan
data <- all
data$C92 <- ifelse(data$f.eid %in% C92$V1, 1, 0)
data$M313 <- ifelse(data$f.eid %in% M313$V1, 1, 0)
```
...
data %>%
    mutate(decile = ntile(PRS, 10)) %>%
    group_by(decile) %>%
    summarize(nb=sum(M313)) %>%
    ggplot( aes(x=decile, y=nb)) +
      geom_bar(stat="identity", width=0.4, fill="skyblue", alpha=0.9) +
      scale_x_continuous(breaks=seq(1,10)) +
      xlab("VitaminD PRS decile") +
      ylab("Number of people with Granulomatosis")
```
