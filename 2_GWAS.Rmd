...
cat CLUMP*clumped | grep -v "CHR" |sed '/^$/d'  >> result_GWAS_bloodpressure_bolt_clumped
```
...
cd /Users/y.holtz/Dropbox/QBI/4_UK_BIOBANK_GWAS_PROJECT/VitaminD-GWAS/0_DATA
scp  y.holtz@delta.imb.uq.edu.au:/home/y.holtz/BLOOD_GWAS/2_CLUMPING/result_GWAS_bloodpressure_*_clumped  .
```
...
```{r, warning=F, message=F}
# Read result
data=read_delim("0_DATA/result_GWAS_bloodpressure_bolt.linear.light.gz", col_names=T, delim=" ")
data$CHR=as.factor(data$CHR)
#data=data[ sample(seq(1,nrow(data)), 10000) , ]
...
don <- data %>% 
  ...
  # keep only SNP with pvalue < 0.05: it makes the plot realisation way faster
  filter( P<0.03 | SNP %in% sample(data$SNP, 10000)) %>%
  ...
  # Add clumping information
  mutate( is_major=ifelse(SNP %in% clump$SNP & P<10e-40, "yes", "no")) %>%
  mutate( is_clump=ifelse(SNP %in% Hit_SNP, "yes", "no"))
